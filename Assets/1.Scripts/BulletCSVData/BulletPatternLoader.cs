...
public class BulletPatternLoader : MonoBehaviour
{
    ...
    [SerializeField]
    private TextAsset nonPatternCSVFile; // 유니티 인스펙터에서 연결할 수 있는 CSV 텍스트 파일

    [Header("파싱된 패턴형 탄막 데이터 리스트")]
    [Tooltip("BulletSpawnData 구조로 파싱된 패턴형 탄막 정보")]
    [SerializeField]
    private List<BulletSpawnData> patternData = new List<BulletSpawnData>(); //게임에 사용될 패턴형 탄막 데이터 리스트
    ...
    [SerializeField]
    private List<BulletSpawnData> nonPatternData = new List<BulletSpawnData>();   // 최종적으로 게임에 사용할 탄막 데이터 리스트(플레이어 인식, 비인식)
    ...
    void ParseNonPatternCSV    (string csv)
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            nonPatternData.Add(data); // 리스트에 추가
        }
    }    
}