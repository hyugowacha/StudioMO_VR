...
public class AnglePatternSpawner : MonoBehaviour
{
    ...
    public void FireAnglePatternBullet(int side, int preset, float fireAngle, float offset)
    {
        ...
        Vector3 fireDir = (Quaternion.Euler(0f, totalAngle, 0f) * baseDir).normalized;
        if(anglePatternBullet != null)
        {
            Quaternion quaternion = Quaternion.LookRotation(new Vector3(fireDir.x, 0, fireDir.z));
            if (PhotonNetwork.InRoom == false)
            {
                Instantiate(anglePatternBullet, spawnPos, quaternion);
            }
            else if (PhotonNetwork.IsMasterClient == true && Resources.Load<GameObject>(anglePatternBullet.name) != null)
            {
                PhotonNetwork.InstantiateRoomObject(anglePatternBullet.name, spawnPos, quaternion);
            }
            EffectPoolManager.Instance.SpawnEffect("PatternBullet_Enable", spawnPos, Quaternion.identity);
        }
    }
}